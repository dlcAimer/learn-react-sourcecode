...
export function canHydrateInstance(
  ...
  if (
    ...
    // dom 节点名称和 HostComponent 节点名称不一致
...
export function hydrateInstance(
  ...
  // domInstance.__reactFiber$hash = fiber
  precacheFiberNode(internalInstanceHandle, instance);
  // TODO: Possibly defer this until the commit phase where all the events
  // get attached.
  // domInstance.__reactProps$hash = props
  ...
  // 比较 dom.attributes 和 props 的差异，如果 dom.attributes 的属性比 props 多，说明服务端添加了额外的属性，此时控制台提示。
  // 注意，在对比过程中，只有服务端和客户端的 children 属性(即文本内容)不同时，控制台才会提示错误，同时在 commit 阶段，客户端会纠正这个错误，以客户端的文本为主。
  // 但是，如果是 id 不同，则客户端并不会纠正。
  // 同时这里面会对一些不被委托到根节点的事件在节点上进行监听。
