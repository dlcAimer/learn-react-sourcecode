...
export function canHydrateInstance(
  ...
  props: Props,
  /**
	 * 是否在根节点或者单例里，具体用作什么目前没看懂
   * enterHydrationState 里设置为 true
   * */
  ...
  while (instance.nodeType === ELEMENT_NODE) {
    ...
    if (element.nodeName.toLowerCase() !== type.toLowerCase()) {
      ...
      // 对可提升节点和第三方脚本/浏览器插件可能插入的节点进行经验性检测
      // 这里涉及到 itemScope/itemProp 以及 react 对于可提升的判断（isHostHoistableType），暂时不深究      
      // 对于第三方脚本/浏览器插件插入别的节点的情况，例如 div，react 目前并不能区分并处理
      switch (type) {
        // case 'title':
...
export function canHydrateTextInstance(
  ...
  while (instance.nodeType !== TEXT_NODE) {
    if (
      ...
      // 只有在根节点或者单例里，也就是最顶部，才允许跳过未匹配节点继续尝试
      return null;
    }
