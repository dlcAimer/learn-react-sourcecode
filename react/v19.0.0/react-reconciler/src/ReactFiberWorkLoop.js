  ...
  if (
    ...
    if (enableUpdaterTracking) {
      if (isDevToolsPresent) {
        /**
         * 这个函数的作用是将一个 fiber 节点和与之相关的 lane 添加到 root.pendingUpdatersLaneMap 中。
         * 这个 map 用于记录每个 lane 对应的 fiber 节点, 也就是记录了每个更新是由哪些 fiber 节点发起的。
         * 这个信息可以在 React DevTools 中用于显示某个更新是由哪个组件发起的。
         */
    ...
    /**
     * 这个函数的目的是检查在测试代码中,是否所有导致 React 状态更新的操作都被包裹在 act(...) 中。
     * act 是 React 提供的一个测试工具函数,用于确保在测试中,所有的 React 状态更新都在 act 的回调函数中执行。
     * 
     * 这个警告的意义在于,它提醒开发者在编写测试代码时,应该遵循 React 的测试规范,将所有的状态更新操作包裹在 act 中。
     * 这样做的好处是,可以确保测试代码中的行为与在浏览器中的行为一致,因为在浏览器中,React 的状态更新也是批量处理的。
     * 如果没有使用 act,可能会导致测试结果与实际情况不一致。
     */
...
export function scheduleInitialHydrationOnRoot(root: FiberRoot, lane: Lane) {
  // 特定场景下简化版 scheduleUpdateOnFiber
...
function completeUnitOfWork(unitOfWork: Fiber): void {
  ...
  do {
    ...
    if (siblingFiber !== null) {
      // 如果有兄弟节点，继续让 workLoop 调度兄弟节点的任务
    ...
    // 没有兄弟节点了，那么向上返回，并且代表父亲节点的 beginWork 完成，开始执行 completeWork
